  ...
  return (
    <div className="max-w-xs mx-auto bg-white rounded-2xl shadow-md p-4 flex flex-col items-center hover:shadow-lg hover:scale-105 transition duration-300">
      ...
      >
        {plataforma && <img
          loading="lazy"
          className="absolute max-w-7 left-21 bottom-23 md:max-w-8 md:left-30 md:bottom-29 rounded-xl "
          src={plataforma}
      ...
      <h2
        title={`${titulo}`}
        className="text-gray-800 font-semibold text-xs  text-center mb-3"
      >
        {titulo && titulo.length > 25 ? `${titulo.slice(0, 25)}...` : titulo}
      </h2>

      <a href={href} target="_blank" rel="noopener noreferrer">
        <button className="bg-[#9a5833] text-white text-sm px-4 py-2 rounded-lg hover:bg-[#eb874e] transition-colors">
          Saiba mais
        </button>
      </a>
    </div>
